...
$debugfile = "convert_HV.debug";
open(DEBUG, ">$debugfile");

open (OUTPUT, '>converted_HV.txt');   #opens output file
...
foreach ($inline) {  #for each line, convert, add stress, and print to converted.txt
    if(!($inline =~ /^Revised/)){
        ...
        foreach my $syl (@syls) { # for each syllable, convert and push into new array
            ...
            #print "\nAfter remove extraneous, syllable is $syl\n\n";
            
            # then convert all syllable types to their appropriate letter-codes:
            if ($syl =~ '^VVC(C)*$') {
                $syl = 'S'; # always closed, regardless of extrametrical-consonant parameter setting
            } elsif ($syl =~ '^VV$') {
                $syl = 'L'; # long
            } elsif ($syl =~ '(^VC(C)*$)') {
                $syl = 'C'; # closed, could potentially become X, depending on parameter setting
            } elsif ($syl =~ '^V$') {
                $syl = 'X'; # short
            }            
            push(@newSyls, $syl);
